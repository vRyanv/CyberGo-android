...
public class VehicleSelectionViewModel extends BaseViewModel {
    ...
    public VehicleSelectionViewModel(@NonNull Application application) {
        super(application);
        vehicle_repo = VehicleRepository.GetInstance();
        app_db_context = AppDBContext.GetInstance(application);
    }

    public void HandleLoadVehicle(){
        List<Vehicle> vehicles = AppDBContext.GetInstance(getApplication()).VehicleDAO().GetVehicleListByStatus(VehicleStatus.ACCEPTED);
        if(!vehicles.isEmpty()){
            ...
            if (response.body().getCode() == StatusCode.OK) {
                ...
                for ( Vehicle vehicle : response.body().vehicle_list) {
                    if(vehicle.status.equals(VehicleStatus.ACCEPTED)){
                        vehicle_accpected_list.add(vehicle);
                    }
                }
                vehicle_list.postValue(vehicle_accpected_list);
